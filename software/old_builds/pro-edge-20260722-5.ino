...
String sysStatus = "Initializing...";  // Tracks the active logic for serial output
...
void run_auto() {
  ...
  if (isReleaseMode && isPostOrgasmBreak) {
    sysStatus = "Post-Orgasm Break Period";
  ...
  if (delta > pLimit) {
    if (!wasOverLimit) {
      ...
      if (isReleaseMode) {
        ...
        if (millis() - lastEdgeTime > currentCooldownMS || currentEdges == 0) {
          sysStatus = "EDGE DETECTED! Break Active.";
          currentEdges++;
          lastEdgeTime = millis();
          ...
          sysStatus = "PENALTY: Edged again before cooldown expired!";
          lastEdgeTime = millis();
          currentCooldownMS = random(8000, 20001);
          motSpeed = 0;
          analogWrite(MOTPIN, 0);
    ...
    // PREVENT VIBRATION WHILE SERVING BREAK
    if (isServingBreak) {
      sysStatus = "Serving post-Edge Timeout (Clenching)";
      motSpeed = 0;
    } else if (isSmoothMode) {
      sysStatus = "Managing Vibrator (Reduced)";
      // Proportional Logic
    ...
    if (isServingBreak) {
      sysStatus = "Serving post-Edge Timeout (Relaxed)";
      motSpeed = 0;
    } else {
      sysStatus = "Managing Vibrator (Normal)";
      if (motSpeed < (float)maxSpeed) motSpeed += motIncrement;
    }
  }
...
void run_state_machine(uint8_t state) {
  if (state >= OPT_SPEED && state <= OPT_POST_ORG_BREAK) {
    sysStatus = "Settings Menu";
  }
  switch (state) {
...
void loop() {
  ...
  if (millis() % PERIOD == 0) {
    ...
    // --- THROTTLED VERBOSE SERIAL TELEMETRY ---
    ...
    if (millis() - lastLogTime > 250) {
      ...
      if (currentState == AUTO) {
        Serial.print(isReleaseMode ? "RELEASE" : "DENIAL");
        Serial.print(isSmoothMode ? "(Smooth)" : "(Abrupt)");
      ...
      // 2. Print Pressure vs Limits
      Serial.print(" Pres: ");
      Serial.print(pressure);
      Serial.print(" (Base avg: ");
      Serial.print(avgPressure);
      Serial.print(", Limit: ");
      Serial.print(avgPressure + pLimit);
      Serial.print(")");

      // 3. Print Motor Output
      ...
      if (currentState == AUTO && isReleaseMode) {
        ...
        Serial.print("/");
        Serial.print(actualTargetEdges);
      }

      // 5. Print the Working Logic Status
      Serial.print(" | Status: ");
